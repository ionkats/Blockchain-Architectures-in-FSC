...
export function initializeWithoutData() {
    ...
    var accounts0 = []
    web3_0.eth.getAccounts().then(async fetchedAccounts => {
                                let promise = new Promise((resolve,reject) => {
                                    resolve(fetchedAccounts)
                                })
                                accounts0 = await promise;
                                console.log(accounts0)
                                console.log(accounts0[0])
                            })
    ...
    stateandsessionContract_0.deploy({data: contractData, arguments: []})
                            .send({from: accounts0[0], gas: '15000000'})
                            .then( (contractCreated) => {
                                console.log('Contract mined for chain 0. Address: ' + contractCreated.options.address)
